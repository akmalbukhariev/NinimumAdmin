...
import { LoadState, PageTitle, Panel, StatusChip, TablePanel, dateTime, money, useL } from '../components/AdminCommon';
...
function usePaged(loader:(page:number,pageSize:number)=>Promise<api.PageData>) {
  ...
  const pagination=<TablePagination component="div" count={data.total} page={page} rowsPerPage={pageSize} onPageChange={(_,p)=>setPage(p)} onRowsPerPageChange={e=>{setPage(0);setPageSize(Number(e.target.value));}} rowsPerPageOptions={[10,20,50,100]} />;
  return {data,loading,error,reload,pagination};
}

export function OrdersPage(){
  const token=useToken(), l=useL(); const [search,setSearch]=useState(''),[status,setStatus]=useState('');
  ...
  const save=async()=>{if(!edit)return; await api.updateOrderStatus(token,edit.id,{status:edit.status,payment_status:edit.payment_status});setEdit(null);await x.reload();};
  return <><PageTitle title={l('Buyurtmalar','Заказы','Orders')} subtitle={l('Buyurtmalar, to‘lov va holatlarni boshqaring.','Управляйте заказами, оплатой и статусами.','Manage orders, payments and statuses.')} />
    <Stack direction={{xs:'column',sm:'row'}} gap={1.5} mb={2}><TextField size="small" label={l('Qidirish','Поиск','Search')} value={search} onChange={e=>setSearch(e.target.value)} /><TextField select size="small" label={l('Holat','Статус','Status')} value={status} onChange={e=>setStatus(e.target.value)} sx={{minWidth:180}}><MenuItem value="">{l('Barchasi','Все','All')}</MenuItem>{['PENDING','CONFIRMED','PREPARING','ON_THE_WAY','DELIVERED','CANCELLED'].map(v=><MenuItem key={v} value={v}>{v}</MenuItem>)}</TextField><Button onClick={x.reload} startIcon={<RefreshRounded/>}>{l('Yangilash','Обновить','Refresh')}</Button></Stack>
    <LoadState loading={x.loading} error={x.error} onRetry={x.reload}><TablePanel><Table><TableHead><TableRow><TableCell>#</TableCell><TableCell>{l('Mijoz','Клиент','Customer')}</TableCell><TableCell>{l('Mahsulot','Товары','Items')}</TableCell><TableCell>{l('Summa','Сумма','Total')}</TableCell><TableCell>{l('To‘lov','Оплата','Payment')}</TableCell><TableCell>{l('Holat','Статус','Status')}</TableCell><TableCell>{l('Vaqt','Время','Time')}</TableCell><TableCell/></TableRow></TableHead><TableBody>{x.data.items.map(r=><TableRow key={r.id} hover><TableCell>{r.order_number}</TableCell><TableCell>{r.customer_name||r.phone_number||'-'}</TableCell><TableCell>{r.product_count}</TableCell><TableCell>{money(r.total_price)}</TableCell><TableCell><StatusChip value={r.payment_status}/></TableCell><TableCell><StatusChip value={r.status}/></TableCell><TableCell>{dateTime(r.ordered_at)}</TableCell><TableCell><Tooltip title="View"><IconButton onClick={()=>void openDetail(r.id)}><VisibilityRounded/></IconButton></Tooltip><IconButton onClick={()=>setEdit({...r})}><EditRounded/></IconButton></TableCell></TableRow>)}</TableBody></Table>{x.pagination}</TablePanel></LoadState>
    <Dialog open={!!edit} onClose={()=>setEdit(null)} fullWidth maxWidth="xs"><DialogTitle>{l('Buyurtma holati','Статус заказа','Order status')}</DialogTitle><DialogContent sx={{pt:'12px!important'}}><Stack gap={2}><TextField select label={l('Buyurtma holati','Статус заказа','Order status')} value={edit?.status||''} onChange={e=>setEdit(v=>v&&({...v,status:e.target.value}))}>{['PENDING','CONFIRMED','PREPARING','ON_THE_WAY','DELIVERED','CANCELLED'].map(v=><MenuItem key={v} value={v}>{v}</MenuItem>)}</TextField><TextField select label={l('To‘lov holati','Статус оплаты','Payment status')} value={edit?.payment_status||''} onChange={e=>setEdit(v=>v&&({...v,payment_status:e.target.value}))}>{['PENDING','PAID','FAILED','CANCELLED'].map(v=><MenuItem key={v} value={v}>{v}</MenuItem>)}</TextField></Stack></DialogContent><DialogActions><Button onClick={()=>setEdit(null)}>{l('Bekor qilish','Отмена','Cancel')}</Button><Button variant="contained" onClick={()=>void save()}>{l('Saqlash','Сохранить','Save')}</Button></DialogActions></Dialog>
  ...
  return <>
    ...
    <Stack direction={{ xs: 'column', md: 'row' }} gap={1.5} mb={2}>
      ...
      <TextField select size="small" label={l('Mavjudlik', 'Активность', 'Active')} value={active} onChange={(event) => setActive(event.target.value)} sx={{ minWidth: 170 }}>
        <MenuItem value="">{l('Barchasi', 'Все', 'All')}</MenuItem><MenuItem value="true">ACTIVE</MenuItem><MenuItem value="false">INACTIVE</MenuItem>
      </TextField>
    </Stack>
    ...
    <Dialog open={Boolean(edit)} onClose={() => setEdit(null)} fullWidth maxWidth="md">
      ...
      <DialogContent sx={{ pt: '12px!important' }}><Box display="grid" gridTemplateColumns={{ xs: '1fr', md: '1fr 1fr' }} gap={2}>
        ...
        <Box gridColumn={{ md: '1 / -1' }}>
          ...
          {Array.isArray(edit?.images) && edit.images.length > 0 && <Stack direction="row" gap={1} flexWrap="wrap" mb={1.5}>{edit.images.map((image: Row) => <Box key={image.id} position="relative"><Avatar variant="rounded" src={api.imageUrl(image.image_url)} sx={{ width: 80, height: 80 }} /><IconButton size="small" color="error" onClick={() => void removeEditImage(Number(image.id))} sx={{ position:'absolute',right:-8,top:-8,bgcolor:'background.paper',boxShadow:1 }}><DeleteRounded fontSize="small" /></IconButton></Box>)}</Stack>}
...
export function CategoriesPage(){
  const token=useToken(),l=useL(); const [rows,setRows]=useState<Row[]>([]),[loading,setLoading]=useState(true),[error,setError]=useState<string|null>(null),[edit,setEdit]=useState<Row|null>(null);
...
export function SubscriptionsPage(){
  ...
  const subStatus=async(r:Row,s:string)=>{try{await api.setSubscriptionStatus(token,r.id,s);await x.reload();}catch(e){setError(e instanceof Error?e.message:'Error')}};
  const blankTariff=()=>({id:0,name:'',monthly_price:'',duration_months:1,description:'',is_active:true});
  const saveTariff=async()=>{if(!editTariff)return;try{if(editTariff.id)await api.updateTariff(token,editTariff.id,editTariff);else await api.createTariff(token,editTariff);setEditTariff(null);await loadTariffs();}catch(e){setError(e instanceof Error?e.message:'Error')}};
  return <>
...
export function DeliveryPage(){
  ...
  const updateJob=async(r:Row,data:Row)=>{try{await api.updateDeliveryJob(token,r.id,data);await x.reload()}catch(e){setError(e instanceof Error?e.message:'Error')}};
  const workerStatus=async(r:Row)=>{try{await api.setDeliveryWorkerStatus(token,r.id,r.status==='ACTIVE'?'INACTIVE':'ACTIVE');await loadWorkers()}catch(e){setError(e instanceof Error?e.message:'Error')}};
...
export function ReviewsPage(){
  ...
  const toggleReview=async(r:Row)=>{try{await api.setReviewActive(token,r.id,!r.is_active);await reviews.reload()}catch(e){setError(e instanceof Error?e.message:'Error')}};
  const toggleQuestion=async(r:Row)=>{try{await api.setQuestionActive(token,r.id,!r.is_active);await questions.reload()}catch(e){setError(e instanceof Error?e.message:'Error')}};
...
export function PromotionsPage(){
  ...
  const savePromo=async()=>{if(!edit)return;try{if(edit.promotion_id)await api.updatePromotion(token,edit.promotion_id,edit);else await api.createPromotion(token,edit);setEdit(null);await load()}catch(e){setError(e instanceof Error?e.message:'Error')}};
  const saveBanners=async()=>{try{await api.replaceBanners(token,bannerIds.map((id,i)=>({product_id:id,sort_order:i+1})));await load()}catch(e){setError(e instanceof Error?e.message:'Error')}};
  ...
  const saveCoupon=async()=>{if(!editCoupon)return;try{if(editCoupon.coupon_id)await api.updateCoupon(token,editCoupon.coupon_id,editCoupon);else await api.createCoupon(token,editCoupon);setEditCoupon(null);await load()}catch(e){setError(e instanceof Error?e.message:'Error')}};
  ...
  return <>
    ...
    <Tabs value={tab} onChange={(_,v)=>setTab(v)} sx={{mb:2}}><Tab label={l('Aksiyalar','Акции','Promotions')}/><Tab label={l('Bannerlar','Баннеры','Banners')}/><Tab label={l('Kuponlar','Купоны','Coupons')}/></Tabs>
